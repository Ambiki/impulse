...
- Removing one token from a multi-token `data-action` attribute no longer stops every listener on that element. Each `@action` listener is now keyed by its own token, so only the removed descriptor is unbound ([#143](https://github.com/Ambiki/impulse/issues/143))
